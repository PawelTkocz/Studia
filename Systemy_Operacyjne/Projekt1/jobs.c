      ...
      for (int i = 0; i < job->nproc; i++) {
        ...
        if (pid != 0) {
          if (WIFEXITED(status) || WIFSIGNALED(status)) {
            // exited or killed by  signal
            proc->exitcode = status;
            proc->state = FINISHED;
            bool job_finished = true;
            for (int i = 0; i < job->nproc; i++) {
              if (job->proc[i].state != FINISHED)
                job_finished = false;
            }
            if (job_finished)
              job->state = FINISHED;
...
int monitorjob(sigset_t *mask) {
  ...
  while (state != STOPPED && state != FINISHED) {
    ...
    if (state == STOPPED) {
      int ind = allocjob();
      movejob(0, ind);
      Tcgetattr(tty_fd, &jobs[ind].tmodes);
    }
